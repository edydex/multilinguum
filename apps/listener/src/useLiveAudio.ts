...
export function useLiveAudio(apiBase: string, availableLanguages: Language[], volume: number) {
  ...
  const availabilityKey = availableLanguages.join(',');
  ...
  const attachSelected = useCallback(() => {
    ...
    if (!wanted.current || !selected.language || !selected.available.includes(selected.language))
      return;
    ...
    void element
      ...
      .then(() => {
        if (
          ...
          selection.current.available.includes(selection.current.language)
        )
          setPlaying(true);
        else element.pause();
      })
      .catch(() => {
        ...
        stop();
        setError('Choose audio again to allow playback.');
      });
  }, [detach, stop]);

  useLayoutEffect(() => {
    selection.current.available = availableLanguages;
    const current = selection.current.language;
    if (current && !availableLanguages.includes(current)) stop();
  ...
  const start = useCallback(
    async (language: Language) => {
      stop();
      if (!selection.current.available.includes(language)) return;
      selection.current.language = language;
      wanted.current = true;
      const attempt = ++generation.current;
      const isCurrent = () =>
        generation.current === attempt &&
        wanted.current &&
        selection.current.available.includes(language);
      setConnecting(true);
      setError(undefined);
      try {
        const { Room: LiveKitRoom, RoomEvent } = await import('livekit-client');
        if (!isCurrent()) return;
        const tokenUrl = publicEndpoint(apiBase, 'token');
    ...
    [apiBase, attachSelected, detach, stop],
  );

  useLayoutEffect(() => {
    const language = selection.current.language;
  ...
  return { connected, playing, connecting, error, start, stop };
}
