...
The live capture route now feeds 48 kHz frames to a shared live transcriber and the active direct translation channels without waiting for a five-second file chunk. Source archive/original-audio publication remains five-second chunked. The source transcriber emits replaceable source-language deltas and commits at a detected 360 ms source pause or an eight-second safety maximum for uninterrupted speech. Expressive text retains context across those recognition windows; it will not release a very short emphasized fragment as a standalone narrator clause. The translation pass produces continuous punctuation, then audio is rendered ahead, edge-trimmed, and queued in source order. Translated drafts are withheld, and the listener reveals scheduled final text no more than 1.8 seconds before playout. Each finalized caption creates an archive latency sample. Server-side latency includes actual LiveKit queue depth, but remains provisional until a listener-side acoustic probe measures sound reaching a phone.
