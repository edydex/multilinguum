...
- GPT-Live-Transcribe deltas now drive replaceable source and translated caption previews. The public listener retains up to 160 finalized clauses in a user-scrollable history, follows the live edge only while the listener remains near the bottom, and schedules approximate word-level karaoke state from measured LiveKit queue and rendered-audio duration.
- The live transcriber closes a recognition window after 360 ms of detected mixer silence, with an eight-second safety maximum for uninterrupted speech. Expressive speech still buffers coherent clauses, but a real preacher pause can end a clause early; a bounded portion of that pause is retained in the translated program while synthetic provider padding is removed.
