...
- The live transcriber closes a recognition window after 360 ms of detected mixer silence, with an eight-second safety maximum for uninterrupted speech. Recognition windows are no longer narration boundaries: isolated emphasis fragments are held for the surrounding thought, and a short rhetorical question such as `What?` is held for the next parallel question instead of being narrated alone. The translation pass returns both continuous narrator-ready text and a constrained semantic delivery plan (`enumeration`, `contrast`, `appeal`, and related roles) with at most three exact emphasis spans. This lets TTS distinguish parallel points such as `What? How? Why?` and place the semantic peak on a word such as `implores`. Coarse source pace, energy, and terminal contour still steer generic TTS delivery; a bounded portion of the real pause is retained while synthetic provider padding is removed.
