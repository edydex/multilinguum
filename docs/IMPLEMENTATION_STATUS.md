...
- GPT-Live-Transcribe deltas drive a replaceable preview only on the delayed source-language channel. Translated drafts are not exposed: a translated caption becomes visible only when rendered audio has a real playout schedule, and the browser limits look-ahead to 1.8 seconds. The listener retains up to 160 finalized clauses in a user-scrollable history, follows the clause whose narration has actually begun, and schedules approximate word-level karaoke state from measured LiveKit queue and rendered-audio duration.
- The live transcriber closes a recognition window after 360 ms of detected mixer silence, with an eight-second safety maximum for uninterrupted speech. Recognition windows are no longer narration boundaries: isolated emphasis fragments are held for the surrounding thought, a short rhetorical question such as `What?` is held for the next parallel question, and one complete thought waits up to 1.8 seconds for a bounded following-source preview. The translation pass returns both continuous narrator-ready text and a constrained target-language semantic director: rhetorical role, cadence, `setup → build → climax → resolution` arc position, target-language pause boundary, at most three exact emphasis spans, and up to five exact delivery beats. This lets TTS distinguish parallel points such as `What? How? Why?` and build the contrast toward `implores`. Source pace and energy are evidence only for broad affect; source-language pitch, word stress, contour, and pause placement are explicitly not copied. Synthetic padding is removed, while the semantic director selects the short connection or completed-thought boundary.
...
- The operator exposes product-facing **Generic · Fast**, **Generic · Expressive**, and named consented-voice modes. Expressive is the glossary-aware accuracy default; cloned voice is locked to finalized glossary-aware clauses. **Add cloned voice…** records consent, hashes and uploads the reference, and marks the profile ready only after encrypted worker installation. Russian and English begin enabled for the current quality pass; Spanish and Ukrainian remain available but opt-in until their latency is tuned. Both generic paths retain the conservative two-hour cloud estimate until measured cascade cost replaces the planning baseline.
